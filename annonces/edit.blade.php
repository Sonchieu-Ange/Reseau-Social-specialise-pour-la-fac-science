...
<div class="card border-0 shadow-card">
    <div class="card-body p-4">
        ...
        <form method="POST" action="{{ route('annonces.update', $annonce->_id) }}">
            @csrf @method('PUT')
            <div class="mb-3">
                <label for="titre" class="form-label fw-medium">Titre <span class="text-danger">*</span></label>
                <input type="text" name="titre" class="form-control @error('titre') is-invalid @enderror" 
                       value="{{ old('titre', $annonce->titre) }}" required placeholder="Titre de l'annonce">
            ...
            <div class="mb-3">
                ...
                <textarea name="contenu" rows="5" class="form-control @error('contenu') is-invalid @enderror" 
                          required placeholder="Contenu de l'annonce...">{{ old('contenu', $annonce->contenu) }}</textarea>
            ...
            <div class="d-flex gap-3 mt-4">
                ...
                <a href="{{ route('annonces.show', $annonce->_id) }}" class="btn btn-outline-secondary px-4 py-2">Annuler</a>
            </div>
        </form>
    </div>
</div>
